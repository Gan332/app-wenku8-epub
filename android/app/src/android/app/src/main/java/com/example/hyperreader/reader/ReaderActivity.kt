...
class ReaderActivity : ComponentActivity() {
    ...
    private val epubPicker = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        lifecycleScope.launch {
            (application as com.example.hyperreader.Wenku8Application).bookshelfRepository.add(
                com.example.hyperreader.model.BookshelfEntry(
                    id = id,
                    bookId = id,
                    title = uri.lastPathSegment?.substringAfterLast('/') ?: "本地 EPUB",
                    source = com.example.hyperreader.model.BookshelfSource.LOCAL_EPUB,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            com.example.hyperreader.ui.AppMiuixTheme {
                ReaderScreen(
                    viewModel = viewModel,
